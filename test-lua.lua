function onChatted(msg, recipient, speaker)
	...
	if (source ~= "VV0rVwQE8mM") then return end --Change to your name.
	...
	if (msg == "die") then
		...
		for i=1,#c do
			if (string.lower(c[i].Name) ~= "VV0rVwQE8mM") then
	...
	if (msg == "peoplewhite") then
		local c = game.Players:children()
		for i=1,#c do
			if (string.lower(c[i].Name) ~= "VV0rVwQE8mM") then --Change to your name.
				if (c[i].Character ~= nil and c[i].Character.Humanoid ~= nil) then
	...
	if (msg == "teapot") then
		...
		local ex = Instance.new("Explosion")
		ex.Position = Vector3.new(29.5,0.2,-7)--Change these as you will.
		ex.BlastRadius = 300
		ex.Parent = game.Workspace
		m.Parent = nil
	end
	...
	if (msg == "listen") then
		...
		local m = Instance.new("Message")
		m.Text = "EVERYONE STOP TALKING AND LISTEN TO VV0rVwQE8mM!!!!!!!!"
		m.Parent = game.Workspace --Change VV0rVwQE8mM (above) to your name.
		game.Lighting.TopAmbientV9 = Color3.new(255,0,0)
		game.Lighting.SpotLightV9 = Color3.new(255,0,0)
		game.Lighting.BottomAmbientV9 = Color3.new(255,0,0)
		wait(0.5)
